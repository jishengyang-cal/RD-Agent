...
- "id": a unique identifier string, such as `data_science/competition_A/trace_1` or `finance/trace_1`, used to mark the series of logs generated by this RD-Agent run.

The local debug replay server reads traces beneath `UI_TRACE_FOLDER` and
remains bound to `127.0.0.1`. See the [UI guide](../../../docs/ui.rst#remote-access-and-authentication)
for the shared authentication contract, browser bootstrap, and security limitations,
and its [configuration reference](../../../docs/ui.rst#configuration) for trace settings.
